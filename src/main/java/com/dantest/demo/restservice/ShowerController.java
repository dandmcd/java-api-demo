...
public class ShowerController {
    ...
    public ResponseEntity<?> updateShowerStatus(@PathVariable int id, @RequestBody ShowerStatusUpdateRequest request) {
        ...
        try {
            showerService.updateShowerStatus(id, request.isOnline(), request.isUnderMaintenance());
        } catch (IOException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to send WebSocket update");
        }
